...
const TiltCard = ({ title, description, imageUrl }: TiltCardProps) => {
  ...
  const rotateX = useTransform(y, [-0.5, 0.5], [15, -15]); // Reduced tilt for better UX
  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{ perspective: 1200, rotateX, rotateY }} // Removed quotes from perspective value
      ...
      >
        ...
        >
          <h2 className="text-xl sm:text-2xl font-bold mb-2 drop-shadow-md">{title}</h2>
          <p className="text-xs sm:text-sm drop-shadow-md">{description}</p>
        </motion.div>
      </motion.div>
    </motion.div>
  );
...
// TiltCardDemo Component (Added for Preview)
const TiltCardDemo: React.FC = () => {
  return (
    ...
    <div className="min-h-screen flex flex-col py-1 px-5 ">
      ...
      <div className="mb-10">
        ...
        <p className="text-base sm:text-lg">
          A 3D tilt card with a gradient border and interactive hover effects.
        </p>
      </div>
